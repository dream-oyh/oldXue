...
def _get_shortcut_path() -> Path:
    """自启动快捷方式完整路径。"""
    return _get_startup_dir() / "薛氏语音助手.lnk"
...
def set_auto_start(enable: bool):
    ...
    if enable:
        ...
        ps_script = (
            ...
            f'$sc.Description = "薛氏语音助手 - PTT 语音转文字"; '
            f'$sc.Save()'
        )
        subprocess.run(
...
class SettingsWindow:
    ...
    def _on_hotkey_key(self, event: tk.Event):
        ...
        if event.state & 0x0040:  # Win (actually this might vary)
