...
PRE_SPEECH_PADDING = 10           # 语音段前保留 10 帧 (~300ms)
...
# 模块级 ONNX 会话 + 元数据（单例）
_vad_session = None
_vad_state_shape = None     # (batch, ...) for state input
_vad_sr_shape = None        # for sr input
...
class _VadProcessor:
    ...
    def process(self, frames: list[np.ndarray]) -> list[bytes]:
        ...
        frames: list of numpy int16 arrays, each of length FRAME_SAMPLES
        返回: list of bytes (raw PCM, 16kHz 16bit mono)
        """
        self._load_model()
        ...
        for start, end in segments:
            ...
            pcm = np.concatenate(seg_frames).tobytes()
            result.append(pcm)
    ...
    def _smooth_speech(self, is_speech: list[bool]) -> list[bool]:
        ...
        min_silence_frames = 125  # 静音 > 4 秒才切分 (125*32ms≈4s)
...
class CaptureSession:
    ...
    def _callback(self, indata: np.ndarray, frames: int, time_info, status):
        ...
        # 达到上限则丢弃新帧（最多录 MAX_RECORD_SECONDS 秒）
        if len(self._frames) >= MAX_FRAMES:
            return

        # indata 已是 int16 (dtype 指定)，shape 为 (FRAME_SAMPLES, 1)
        frame = indata[:, 0].copy()
