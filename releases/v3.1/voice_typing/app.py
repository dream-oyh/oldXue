...
class VoiceTypingApp:
    ...
    def _show_usage_tip(self):
        """在独立线程中弹出使用说明窗口。"""
        def _run():
            from .config import get_output_method_name
            ...
            info = [
                ("快捷键", hotkey),
                ("识别引擎", engine_name),
                ("输出方式", get_output_method_name(self.config.get("output_method", "unicode"))),
            ]
    ...
    def _message_loop(self):
        ...
        # 无云端 API 时，一次性提示长语音限制
        has_cloud_api = all([
            self.config.get("app_id", "").strip(),
            self.config.get("api_key", "").strip(),
            self.config.get("api_secret", "").strip(),
        ])
        if not has_cloud_api and not self.config.get("long_audio_cloud_hint_shown", False):
            ...
            cfg = load_config()
            cfg["long_audio_cloud_hint_shown"] = True
            save_config(cfg)
            self.config = cfg
